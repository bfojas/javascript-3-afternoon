...
//Code Here
let evenNumbers =mixedNumbers.filter((val,i,arr) => arr[i]%2 === 0)
...
//Code Here
let postTaxPrices  = prices.map((val,i,arr) => val *= 1.07)
...
//Code Here
let totalPopulation = populations.reduce((accumulator, val, i, arr) => accumulator += arr[i])
...
let orderTotals  = orders.map((val,i,arr) => val.price * (1+val.tax))// Code here
...
let bobsTotal = purchases.filter((val)=> val.owner === 'Bob').reduce((acc,val,i,arr) => 
  acc +=arr[i].price, 0)
